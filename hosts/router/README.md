...
```
        ISP
         |
     enp1s0 (WAN, DHCP client)
         |
    +---------+
    |  APU4D4 |
    +---------+
         |
     enp2s0 (single flat LAN, 10.10.0.0/24)
         |
    unmanaged switch(es)
      /         |         \
 wired PCs   AX57 (AP)   AX55 (AP)
             SSID: home   SSID: home
             SSID: guest  SSID: guest  (each AP's own built-in isolated
                                        Guest Network, independent of
...
So instead: one flat `10.10.0.0/24`, with devices sorted into IP *ranges*
by MAC address rather than VLANs by tag:

| Range | Addresses | Who |
|---|---|---|
| trusted | `10.10.0.10` – `.99` | devices you've explicitly added by MAC |
| iot | `10.10.0.100` – `.149` | devices you've explicitly added by MAC |
| fallback (guest-equivalent) | `10.10.0.150` – `.250` | anything with an unrecognized MAC |
...
- Per-range policy for anything that terminates at or transits through
  the router: only `trusted` can SSH into the router; every range gets
  outbound internet.
...
- Isolation *between* devices on the same wire. Two devices on the same
  ...
  in the `iot` range can still directly probe a laptop in `trusted` if
...
4. After it reboots into AP mode, it'll request an address from your
   router's dnsmasq — check `10.10.0.150`–`.250` for it, or add its MAC to
   `dhcp-host` in `router.nix` to give it a stable trusted-range address
   (recommended, since you'll want to reach its admin UI reliably).
...
7. For a guest-style network on the same AP: **Wireless → Guest Network**,
   ...
   - Note this isolates that guest SSID from *this AP's own* bridge only —
     it doesn't know about or interact with the trusted/iot ranges on the
     router. Treat it as a second, independent layer, not a substitute for
     the router-side MAC assignment.
...
1. **Fix the interface names.** Boot a NixOS live USB on the actual box
   ...
   `hosts/router/router.nix` accordingly.
...
- If a device isn't getting the static IP you assigned, double check the
  ...
  fallback range, which can look like "it's ignoring my config" when it's
  actually working exactly as designed (unrecognized MAC → fallback).
